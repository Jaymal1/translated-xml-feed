...
def save_raw_xml():
    ...
    content = response.text.lstrip('\ufeff').strip()
    if not content.startswith("<?xml"):
        raise Exception("Response does not appear to be valid XML.")
...
def get_existing_barcodes(tree):
    return {opt.find("Barkod").text for opt in tree.findall(".//Secenek") 
            if opt.find("Barkod") is not None and opt.find("Barkod").text}

def translate_text_tr_to_en(text):
    # Dummy translator; replace with actual translator call if needed
    return text
...
def process_products(raw_tree, output_tree, rate):
    ...
    for urun in raw_tree.findall(".//Urun"):
        ...
        # Copy fields, including OzelAlan2 without translation
        for field in ["UrunKartiID", "UrunAdi", "Aciklama", "Marka",
                      "Kategori", "KategoriTree", "UrunUrl", "OzelAlan2"]:
            # For UrunAdi and Aciklama, you can translate if desired; here we keep original
            text = urun.findtext(field, "")
            ET.SubElement(urun_kopya, field).text = text
        ...
        if urunsecenek_tag is not None:
            ...
            for secenek in urunsecenek_tag.findall("Secenek"):
                barkod = secenek.findtext("Barkod", "").strip()
                secenek_out = ET.SubElement(secenek_group, "Secenek")
                for child in ["VaryasyonID", "StokKodu", "Barkod", "StokAdedi"]:
                    ET.SubElement(secenek_out, child).text = secenek.findtext(child, "")

                # Price conversion
                satis = secenek.findtext("SatisFiyati", "0").replace(",", ".")
                try:
                    price_usd = round(float(satis) * rate, 2)
                except:
                    price_usd = 0.0
                ET.SubElement(secenek_out, "PriceUSD").text = str(price_usd)

                # Copy EkSecenekOzellik as-is
                ek = secenek.find("EkSecenekOzellik")
                if ek is not None:
                    new_ek = ET.SubElement(secenek_out, "EkSecenekOzellik")
                    for oz in ek.findall("Ozellik"):
                        new = ET.SubElement(new_ek, "Ozellik", oz.attrib)
                        new.text = oz.text
        ...
        if barkod and barkod not in barcodes_out:
            ...
            for existing_sec in root_out.findall(".//Secenek"):
                if existing_sec.findtext("Barkod", "") == barkod:
                    existing_sec.find("StokAdedi").text = secenek.findtext("StokAdedi", "0")
                    existing_sec.find("PriceUSD").text = str(price_usd)
...
def main():
    ...
    process_products(raw_tree, output_tree, rate)
    output_tree.write(OUTPUT_XML_PATH, encoding="utf-8", xml_declaration=True)
    print("Done. Output saved to", OUTPUT_XML_PATH)
